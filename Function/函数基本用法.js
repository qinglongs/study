 ...
 * 3.使用参数默认值时，不能有重名参数。**因为指定参数默认值后，会形成一个单独的作用域，就相当于在重复声明一个变量了**
...
/**
 * 下面代码中，参数p的默认值是x+1.这时，每次调用函数foo，都会计算x+1，而不是默认p等于100
 */
...
/**
 * 下面代码，只使用了对象的解构赋值默认值，没有使用函数参数的默认值。只有当函数foo的参数
